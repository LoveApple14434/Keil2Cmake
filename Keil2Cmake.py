...
def edit_config(edit_string):
    ...
    # 验证键是否有效
    valid_keys = ['ARMCC_PATH', 'ARMCLANG_PATH', 'GNU_PATH']
...
def get_toolchain_path(toolchain_type):
    ...
    if key in config['TOOLCHAINS']:
        return config['TOOLCHAINS'][key]
    
    # 回退到默认值
    defaults = {
        'ARMCC': 'D:/Program/Keil_v5/ARM/ARM_Compiler_5.06u7/bin/',
        'ARMCLANG': 'D:/Program/Keil_v5/ARM/ARMCLANG/bin/',
    }
    return defaults.get(toolchain_type, '')
...
def generate_scatter_file(destination:str):
    templateScatter = """
; *************************************************************
...
def main():
    ...
    parser.add_argument('--toolchain', 
                        choices=['ARMCC', 'ARMCLANG', 'GNU'], 
                        default=None,
                        help='Toolchain to use: ARMCC, ARMCLANG or GNU Arm Embedded Toolchain')
    parser.add_argument('-e', '--edit', 
                        help='Edit toolchain path in format KEY=VALUE (e.g., ARMCC_PATH=D:/path/to/compiler)')
    parser.add_argument('-sc', '--show-config', action='store_true',
                        help='Show current toolchain paths')
